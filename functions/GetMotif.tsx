...
// Function to round the vertex positions to a specific precision
function roundPositions(positions: number[], precision: number): number[] {
  ...
  async function addChildrenFromJson(
      ...
      scene: BABYLON.Scene,
      precision: number = 1  // Add precision as a parameter
  ): Promise<number[]> {
      ...
      keys.forEach((key) => {
        if (motifJSONFileData[key].length === 4) {
          ...
          // Reduce precision of vertex positions
          const roundedBackbonePositions = roundPositions(backbonePositions, precision);
          const roundedRingPositions = roundPositions(ringPositions, precision);
          ...
          backboneVertexData.positions = roundedBackbonePositions;
          ...
          ringVertexData.positions = roundedRingPositions;
          ...
          center[0] += roundedRingPositions[0];
          center[1] += roundedRingPositions[1];
          center[2] += roundedRingPositions[2];
